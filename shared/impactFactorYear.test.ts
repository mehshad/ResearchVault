...
test("an absent or malformed cut-off falls back to the calendar year", () => {
  ...
  assert.equal(effectiveImpactFactorYear(on("2026-03-01"), "13-01"), 2026);
  assert.equal(effectiveImpactFactorYear(on("2026-03-01"), "06-32"), 2026);
});
...
test("before a 30 June cut-off a manuscript counts as the previous year", () => {
  ...
  assert.equal(effectiveImpactFactorYear(on("2026-03-15"), "06-30"), 2025);
  assert.equal(effectiveImpactFactorYear(on("2026-06-29"), "06-30"), 2025);
});

test("on and after the cut-off it counts as its own year", () => {
  assert.equal(effectiveImpactFactorYear(on("2026-06-30"), "06-30"), 2026);
  assert.equal(effectiveImpactFactorYear(on("2026-12-31"), "06-30"), 2026);
});
...
test("publication year means the effective year, cut-off included", () => {
  const settings = { impactFactorYear: "publication" as const, impactFactorCutoff: "06-30" };
...
test("prior year steps back from the effective year, not the calendar one", () => {
  const settings = { impactFactorYear: "prior" as const, impactFactorCutoff: "06-30" };
...
test("latest ignores the cut-off entirely", () => {
  ...
  const settings = { impactFactorYear: "latest" as const, impactFactorCutoff: "06-30" };
...
test("a cut-off must be a real month and day", () => {
  for (const good of ["01-01", "06-30", "12-31", "02-29"]) {
    assert.equal(isValidImpactFactorCutoff(good), true, good);
  }
  for (const bad of ["1-1", "00-10", "13-01", "06-00", "06-32", "june", "", null, 630]) {
...
test("the examples straddle the cut-off and show the jump", () => {
  const examples = impactFactorExamples(
    { impactFactorYear: "publication", impactFactorCutoff: "06-30" },
    2026,
  );
...
test("the examples follow the prior-year setting too", () => {
  const examples = impactFactorExamples(
    { impactFactorYear: "prior", impactFactorCutoff: "06-30" },
    2026,
  );
...
test("with latest, every example uses the current year", () => {
  const examples = impactFactorExamples(
    { impactFactorYear: "latest", impactFactorCutoff: "06-30" },
    2026,
  );
...
test("the examples are computed, not written down", () => {
  ...
  const june = impactFactorExamples({ impactFactorYear: "publication", impactFactorCutoff: "06-30" }, 2026);
  const march = impactFactorExamples({ impactFactorYear: "publication", impactFactorCutoff: "03-01" }, 2026);
