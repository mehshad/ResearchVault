...
export default function IbcOfficePage() {
  ...
  const { data: applications = [], isLoading: applicationsLoading } = useQuery<IbcApplication[]>({
    queryKey: ["/api/ibc-applications"],
  });

  const { data: boardMembers = [], isLoading: boardMembersLoading } = useQuery<(IbcBoardMember & { scientist: Scientist })[]>({
